        ...
        <div className="grid gap-6 md:grid-cols-3">
          {benefits.map(({ icon: Icon, metric, unit, title, subtext, detail, delay }) => (
            <motion.div
              ...
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-[#D3D1C7] bg-white p-8 transition-shadow duration-300 hover:border-[#1D9E75]/40 hover:shadow-[0_12px_32px_rgba(0,0,0,0.08)]"
            >
              ...
              {/* Icon */}
              <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#E1F5EE]">
                <Icon size={24} className="text-[#1D9E75]" strokeWidth={1.5} />
              </div>

              {/* Metric */}
              <div className="mb-1 flex items-end gap-2">
                <span className="text-5xl font-bold tracking-[-0.03em] text-[#2C2C2A]">
                  {metric}
                </span>
              ...
              {/* Title + subtext */}
              <h3 className="mb-2 text-lg font-semibold text-[#2C2C2A]">{title}</h3>
              <p className="mb-4 text-sm leading-relaxed text-[#5F5E5A]">{subtext}</p>

              {/* Detail pill */}
              <div className="mt-auto rounded-lg border border-[#D3D1C7] bg-[#F1EFE8] px-3 py-2">
                <p className="text-xs text-[#5F5E5A]">{detail}</p>
              </div>
            </motion.div>
          ))}
        </div>
